 ...
 */
void *
memcpy1(void *s1, const void *s2, size_t n)
{
...
void sha_update(SHA_INFO *sha_info, BYTE *buffer, int count)
{
	...
	memcpy1(sha_info->data, buffer, SHA_BLOCKSIZE);
    ...
    memcpy1(sha_info->data, buffer, count);
}
...
SHA_INFO sha_info1, sha_info2;
BYTE buffer1[BLOCK_SIZE], buffer2[BLOCK_SIZE];

void sha_update_hls() {
    int i;
    int ITERS=5;
    sha_info1.digest[0] = 0x67452301L;
    sha_info1.digest[1] = 0xefcdab89L;
    sha_info1.digest[2] = 0x98badcfeL;
    sha_info1.digest[3] = 0x10325476L;
    sha_info1.digest[4] = 0xc3d2e1f0L;
    sha_info1.count_lo = 0L;
    sha_info1.count_hi = 0L;
    for (i=0;i<ITERS;i++) {
#pragma HLS UNROLL
	    sha_update(&sha_info1, buffer1, BLOCK_SIZE);
    }
    //sha_update(&sha_info2, buffer2, BLOCK_SIZE);
}
